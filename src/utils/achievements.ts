...
export const updateAchievementProgress = (
  ...
  const allEntries = [...diaryEntries, newEntry].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
...
// 연속 스트릭 계산
const calculateConsecutiveStreak = (entries: DiaryEntry[], keywords: string[]): number => {
...
// 총 개수 계산
const calculateTotalCount = (entries: DiaryEntry[], keywords: string[]): number => {
...
// 키워드 매칭 확인
const hasMatchingKeywords = (entry: DiaryEntry, keywords: string[]): boolean => {
  if (keywords.length === 0) return true

  const content = entry.content.toLowerCase()
  const extractedActions = entry.extractedActions.map(action => action.toLowerCase())
